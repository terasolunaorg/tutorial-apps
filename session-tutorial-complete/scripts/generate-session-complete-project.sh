...
# generate session tutorial complete apps
case "${ARCHETYPE_VERSION:0:5}" in
  5.2* | 5.1* | 5.0* )
    bash ./session-tutorial-complete/scripts/generate-project-from-nexus.sh;;
  * )
    if test `echo ${ARCHETYPE_VERSION} | tail -c8 ` = "RELEASE"; then
      bash ./session-tutorial-complete/scripts/generate-project-from-maven-central.sh
    else
      bash ./session-tutorial-complete/scripts/generate-project-from-nexus.sh
    fi;;
esac
